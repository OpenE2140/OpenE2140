...
public class SpawnWeaponsWarhead : EffectWarhead, IRulesetLoaded<WeaponInfo>
{
	...
	public override void DoImpact(in Target target, WarheadArgs args)
	{
		...
		{
			var weaponImpactPosition = new WPos(target.CenterPosition.X, target.CenterPosition.Y, Math.Max(0, target.CenterPosition.Z));
			...
			weaponImpactPosition += WVec.FromPDF(world.SharedRandom, 2) * inaccuracy.Length / 1024;

			for (var j = 0; pieces > j; j++)
			{
				var rotation = WRot.FromYaw(new WAngle(world.SharedRandom.Next(1024)));
				var position = weaponImpactPosition;
				...
				var projectileArgs = new
				{
					...
					Args = new ProjectileArgs
					{
						...
						PassiveTarget = position + new WVec(range, 0, 0).Rotate(rotation)
					}
				};
