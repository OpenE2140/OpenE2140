...
public class SheetBaker
{
    ...
    public byte[] Bake(out int width, out int height, out int offsetX, out int offsetY)
    {
        ...
        var location = new int2(
            this.Frames.Min(f => f.Rectangle.Left),
            this.Frames.Min(f => f.Rectangle.Top)
        );

        var size = new Size(
            this.Frames.Max(f => f.Rectangle.Right - location.X),
            this.Frames.Max(f => f.Rectangle.Bottom - location.Y)
        );
            ...
            for (var y = 0; y < frame.Rectangle.Height; y++)
            {
                Array.Copy(
                    ...
                    (
                        ...
                        i % framesX * frameRectangle.Width - frameRectangle.X + frame.Rectangle.X
                    ) * this.channels,
