...
public sealed class OpenE2140MapGeneratorInfo : MapGeneratorBaseInfo
{
	...
	public override Map Generate(ModData modData, MapGenerationArgs args)
	{
		...
		var param = new Parameters(map, this.GenerateParameterYaml(modData, args));
		...
		CellLayer<bool> playable;
		{
			...
			if (param.ExternalCircularBias > 0)
				poison = terraformer.CenteredCircle(false, true, CellLayerUtils.Radius(map.Tiles) - new WDist(1024));
...
public class OpenE2140MapGenerator : MapGeneratorBase
{
	public OpenE2140MapGenerator(ActorInitializer init, MapGeneratorBaseInfo info)
		: base(init, info)
	{
	}
}
