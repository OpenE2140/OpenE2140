...
public class TerrainTileAnimationInfo : TraitInfo, IRulesetLoaded, ILobbyCustomRulesIgnore
{
	...
	[Desc($"Delays (in ticks) before each loop. Number of delays must be equal or less than {nameof(LoopCount)}.")]
	...
	[Desc($"Change effect position each tick. List of triples (x,y,z) for each loop. ",
		$"Number of items must be equal or less than {nameof(LoopCount)}. By default effect does not move.")]
...
public class TerrainTileAnimation : ITick
{
	...
	void ITick.Tick(Actor self)
	{
		...
		if (--this.ticks <= 0)
		{
			this.ticks = Common.Util.RandomInRange(world.LocalRandom, this.info.Interval);
	...
	private class TileAnimation
	{
		...
		public TileAnimation(TerrainTileAnimationInfo info, WPos initialPosition)
		{
			...
			this.ticks = info.LoopDelays.GetElementOrDefault(0);
		}

		public void Update(World world)
		{
			if (this.effect == null && this.ticks-- <= 0)
			{
				WPos Position() => this.initialPosition + this.info.EffectMovement.GetElementOrDefault(this.loop, WVec.Zero) * ++this.ticks;

				this.effect = new SpriteEffect(Position, () => WAngle.Zero, world, this.info.Image,
					this.info.Sequences[this.loop],
					this.info.Palette);
			...
			{
				this.loop++;
				this.ticks = this.info.LoopDelays.GetElementOrDefault(this.loop);
				this.effect = null;
			}
		}
	}
