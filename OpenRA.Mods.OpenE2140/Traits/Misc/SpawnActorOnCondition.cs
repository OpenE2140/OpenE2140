...
public class SpawnActorOnConditionInfo : TraitInfo, IEditorActorOptions
{
	...
	public readonly Dictionary<string, string> ActorConditions = new();

	[ConsumedConditionReference]
	public IEnumerable<string> LinterSpawnConditions => this.ActorConditions.Values;
...
public class SpawnActorOnCondition : IObservesVariables
{
	...
	private readonly Dictionary<string, BooleanExpression> expressions;
	...
	public SpawnActorOnCondition(ActorInitializer init, SpawnActorOnConditionInfo info)
	{
		...
		this.expressions = this.info.ActorConditions.ToDictionary(p => p.Key, p => new BooleanExpression(p.Value));
	}

	IEnumerable<VariableObserver> IObservesVariables.GetVariableObservers()
	{
		foreach (var (actor, expression) in this.expressions)
		{
	...
	private void RequiredConditionsChanged(Actor self, IReadOnlyDictionary<string, int> conditions, string actor)
	{
		...
		var expression = this.expressions[actor];
